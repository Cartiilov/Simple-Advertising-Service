...
const CreateAPostForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await useCreatePost(title, content);
      setTitle('');
      setContent('');
      ...
      try{
        if(photo)
          console.log("before calling createPhoto")
          await createPhoto(posts[0].id, photo);
      }
      catch (error) {
        console.error(error);
    ...
    <div className={styles.createPostForm} >
        <form onSubmit={handleSubmit}>
            ...
            <div className={styles.buttonAndStatusContainer}>
                <Button buttonStyle='btn--outline2' type="submit" onClick={undefined} buttonSize={undefined}>Submit</Button>
